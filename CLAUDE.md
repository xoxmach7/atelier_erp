...
- **Landing page** (landing.html, корень репо, не в git) — редизайн под ui-ux-pro-max design system (палитра B2B Service `#0369A1`/`#0F172A`, шрифт Plus Jakarta Sans, секция сравнения «без системы vs Sheber Atelier», a11y-доработки). Ребрендинг Sheber ERP → **Sheber Atelier** на лендинге. Задеплоено отдельным Vercel-проектом: https://sheber-atelier-landing.vercel.app (аккаунт bohemetextile8-2199, без git-интеграции — редеплой руками через `vercel --prod` из копии файлов). Логотип: `media/newlogo.webp`.
