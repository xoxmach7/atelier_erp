...
function DesignerCard({ task, mode }: { task: DesignerTask; mode: "measurement" | "quote" | "progress" }) {
  const nextStep = mode === "measurement" ? "Добавить замер" : mode === "quote" ? "Создать КП" : "Открыть заказ";
    ...
    <Card className="border-slate-200 bg-white shadow-sm">
      <CardContent className="space-y-3 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {mode === "quote" ? <Button asChild size="sm"><Link href={task.estimate_url}>{nextStep}</Link></Button> : null}
